...
import { selectedTravelPackages, type ITravelPackage, }  from "../redux/slices/Travel/TravelSlice";
...
const Home: React.FC = () => {
  ...
  const handleMarkerClick = useCallback(
    (packageId: string, ) => {
      ...
      const pkg = travelPackages.travelPackages.find((p: { id: string; }) => p.id === packageId);
  ...
  useEffect(() => {
    dispatch(
      fetchTravelPackagesApi({
        status: "active",
        select:
          "title,category,dateAvailabilities,geoLocationLat,geoLocationLng",
      })
    );
    dispatch(
      fetchTravelPackagesApi({
  ...
  return (
    <div className="flex flex-col justify-center">
      ...
      <PackageModal ref={modalRef} id={active && active.id || ""} onClose={() => setActive(null)} />
